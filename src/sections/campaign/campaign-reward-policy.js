...
export default function CampaignRewardPolicy({ rewardPolicy }) {
  ...
  useEffect(() => {
    if (dataTopReward && dataTopReward.length > 0) {
      ...
      const totalTopReward = dataTopReward.reduce((acc, curr) => (
  ...
  return (
    <Stack spacing={1}>
      <Grid sx={{ mb: 1 }} container spacing={2}>
        <BannerItem title="Campaign budget" number={`${fNumber(your_budget)} ${airTokenName}`} imgSrc="/assets/icons/glass/ic_glass_bag.png" color="info" />
        <BannerItem title="Number of prizes" number={fNumber(airdrop_prizes)} imgSrc="/assets/icons/glass/ic_glass_users.png" color="info" />
                    ...
                    <TableBody>
                      {airdropTableData.map((row) => (
                        <TableRow key={row.level}>
                          ...
                          <TableCell align="center">
                            <Typography variant="caption" sx={{ fontWeight: 'fontWeightSemiBold', color: 'primary.main' }}>
      ...
      <Grid sx={{ mt: 1 }} container spacing={2}>
        <BannerItem title="Affiliates Budget" number={`${fNumber(totalDepositAff)} ${airTokenName}`} imgSrc="/assets/icons/glass/ic_glass_users.png" color="warning" />
        <BannerItem title="Affiliates Level" number={affiliates_level} imgSrc="/assets/icons/glass/ic_glass_users.png" color="warning" />
        <Grid xs={12} sm={12} md={12}>
          <Card sx={{ mt: 1 }}>
            <TableContainer >
              <Scrollbar>
                    ...
                    {level.map((row) => (
                      <TableRow key={row.level}>
                        ...
                        <TableCell align="left">
                          <Typography variant="caption" sx={{ fontSize: 11 }}>{row.explanation}</Typography>
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
